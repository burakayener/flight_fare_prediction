...
def base_models(X, y, scoring="r2"):
    ...
    for name, regress in regressors:
        cv_results = cross_validate(regress, X, y, cv=5, scoring=scoring)
...
def voting_regressor(X, y, best_models):
    ...
    print("Voting Regressor Scores" ,'\n' , -(scores["test_neg_mean_squared_error"].mean()), scores["test_r2"].mean(), -(scores["test_neg_mean_absolute_error"].mean()))
    return voting_reg 